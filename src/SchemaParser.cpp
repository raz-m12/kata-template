...
namespace argskata {
namespace lib {
auto SchemaParser::Parse(const string &bareSchema, const vector<string> &args)
    -> void {
...
auto SchemaParser::AssertValidSchemaFormat(const string &bareSchema,
...
auto SchemaParser::SchemaStartsAndEndsWithParenthesis(const string &schema)
    -> bool {
  return schema.at(0) == '(' && schema.at(schema.length() - 1) == ')';
}

auto SchemaParser::SchemaIsEmptyAndThereAreArgumentsToParse(
    const string &schema, const vector<string> &args) -> bool {
  return schema.length() == 2 && !args.empty();
}

auto SchemaParser::PopulateSchemaWithArguments(const string &bareSchema)
    -> void {
...
auto SchemaParser::IsStringArrayType(const string &token) -> bool {
  return token.length() == 4 && token.at(1) == '[' && token.at(2) == '*' &&
         token.at(3) == ']';
}
...
auto SchemaParser::PopulateArgumentsWithValues(const vector<string> &args)
    -> void {
...
auto SchemaParser::StripNonAlphaChars(const vector<string> &args)
    -> vector<string> {
...
auto SchemaParser::GetAbstractArg(const string &argName)
    -> shared_ptr<AbstractArgument> & {
...
auto SchemaParser::GetStringArrayArgument(const string &argName) const
    -> vector<string> {
