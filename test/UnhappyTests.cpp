...
TEST_F(AnUnhappyTest, ThrowsWhenSchemaDoesNotStartWithOpenParanthesis) {
  ASSERT_THROW(ArgsParser parser("f)", EmptyArgs),
               SchemaMustStartAndEndWithParenthesisException);
}
...
/** Although empty schema is allowed, empty schema with arguments to parse is
 * forbidden. */
TEST_F(AnUnhappyTest, ThrowsWhenSchemaIsEmptyAndThereAreArguments) {
  ...
  ASSERT_THROW(ArgsParser parser(SchemaIsEmpty, {ArgumentNotInSchema}),
               ArgumentNotPartOfSchemaException);
}
...
TEST_F(AnUnhappyTest, ThrowsWhenArgumentIsNotPartOfSchema) {
  ...
  ASSERT_THROW(ArgsParser(Schema, {ArgumentNotInSchema}),
               ArgumentNotPartOfSchemaException);
}

TEST_F(AnUnhappyTest, ThrowsOnInvalidSchemaCharacter) {
  const string schema{"(fd)"};
  ASSERT_THROW(ArgsParser(schema, EmptyArgs),
               SchemaInvalidCharacterDetectedException);
}
